...
guest_email = os.environ.get('DJANGO_GUEST_EMAIL', 'guest@collabeya.com')
...
if not User.objects.filter(username=guest_username).exists():
    ...
    guest_user = User.objects.create_user(username=guest_username, email='guest@collabeya.com', password=guest_password)
